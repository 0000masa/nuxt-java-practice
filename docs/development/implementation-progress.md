...
- **フェーズ17 を実装した(一部実機検証済み)**(2026-09-06 / 実機確認 2026-09-07):
  ...
  - **実機で踏んだこと**(2026-09-07。詳細 → [slack-block-kit-response.md](../notes/aws-code-service/slack-block-kit-response.md)):
    - **承認待ちが残っていると、次の実行では承認通知が飛ばない。** `Approve` ステージを
      ...
      開始されない = `manual-approval-needed` が発火しない。`ExecutionMode: SUPERSEDED` は
      **ステージに入る前で待っている実行**しか追い越さないので、承認待ちは追い越せない。
      **古い承認を承認・却下・中止して片付けるまで、何度リリースしても静かなまま**になる
    - **Block Kit では HTTP 応答本文でメッセージを差し替えられない。** `blocks` を使う場合、
      応答本文は読まれず 200 の合図にしかならない(差し替わるのは attachments 時代の挙動)。
      **エラーも出ないので「却下は CodePipeline に通るのに Slack のメッセージだけ変わらない」**
    ...
    - **却下すると実行は FAILED で終わる。これは正常。** 手動承認に「却下」という終了状態は
      ...
      **2026-09-07 に実機で確認済み**。却下で出るのは「押したメッセージの差し替え」と
      「却下カード 1 通」の 2 つだけになり、`:x:` は出なくなった
