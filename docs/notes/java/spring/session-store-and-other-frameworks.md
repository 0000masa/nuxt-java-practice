...
> **【2026-09-12 追記】このプロジェクトは③から④へ移った。**
> フェーズ18 でセッションの保存先を MySQL から Redis(ElastiCache)にした
> (→ [ADR-0015](../../../adr/0015-session-store-on-redis.md))。
> **以下の③に関する記述は「当時の現在地」ではなく「③という選択肢の分析」として読むこと。**
> 内容自体は Spring Session JDBC の説明として今も正しい。
> 移った後に何が起きたかは「[④に移って実際に何が増えたか](#④に移って実際に何が増えたか)」に書いた。
> Redis 側の詳細 → [docs/notes/redis/](../../redis/)

## まず結論(3 行)

1. **`getSession()` 自体は保存先を知らない。** サーブレット仕様は「このリクエストのセッションを返せ」としか決めていない。MySQL から取り出しているのは、**リクエストを横取りして包み直しているフィルタ**のほう。
2. **保存先は 5 段階あり、このプロジェクトは④インメモリストアにいる**(フェーズ18 まで③共有 RDB にいた)。そして「番号が大きいほど高級」ではない。**①②は複数インスタンスで破綻し、⑤は失効できない。**③と④の差は性能とインフラ費用だけで、**このプロジェクトが④に移った理由も性能ではない**(→ [ADR-0015](../../../adr/0015-session-store-on-redis.md))。
...
| **③** | **共有 RDB** | MySQL / RDS | 残る | 可 | **可** | **不要**(既存 DB に相乗り) | **このプロジェクト(〜フェーズ17)**、Laravel の現在の既定 |
| **④** | **インメモリストア** | Redis / Memcached | 設定次第 | 可 | **可** | **要る**(ElastiCache 等) | **このプロジェクト(フェーズ18〜)**、実務の定番 |
...
- **③と④の違いは性能とインフラ費用だけ。** できること(共有・失効・検索)は同じ。だから ADR-0002 は「性能が問題になったら差し替える」と書けた。
  ただし**実際に④へ移った理由は性能ではなかった**。「実務で使われている構成を手元で再現して理解する」という
  目的が後から加わり、③の利点(コンテナを増やさない・`SELECT` で見える)がその目的の前では
  利点でなくなったため(→ [ADR-0015](../../../adr/0015-session-store-on-redis.md))。
  **同じ 2 つの選択肢を、評価軸が変わると逆に選ぶ**という例として読める。
...
> 以下 2 節は **③を選んだ場合の損得**の分析。このプロジェクトはフェーズ17 までここにいた。
...
現在の本番想定は **Route53 → ALB → ECS Fargate → RDS + ElastiCache**(→ [docs/infrastructure/README.md](../../../infrastructure/README.md))。
...
| ③ 共有 RDB(〜フェーズ17) | **なし** | 不要 | 増えない | 維持される |
| **④ Redis(現在)** | ElastiCache クラスタ、サブネットグループ、専用セキュリティグループ、パラメータグループ、接続情報の受け渡し | 不要 | **常時課金が増える** | 維持される |
...
上の予告どおりに④へ移した(フェーズ18)ので、**予告と実際を突き合わせておく。**

### 予告が当たっていたところ

- **アプリコードは 1 行も変わらなかった。** `UserSessionManager` が
  `FindByIndexNameSessionRepository` にしか依存していなかったため、予告どおり依存 1 行の差し替えで済んだ。
  「抽象より下に依存させない」という指示が実際に効いた
- **ALB は素直なまま。** スティッキーセッションは要らない
- **インフラは増えた。** CloudFormation に **リソース 8 つ**
  (ReplicationGroup / SubnetGroup / ParameterGroup / SecurityGroup / SNS トピック /
  Chatbot チャンネル設定 / アラーム 2 本)

### 予告に書いていなかったところ

| 出てきたもの | 中身 |
| --- | --- |
| **設定キーの変更** | Spring Boot 4 で `spring.session.redis.*` が `level: error` で deprecated になっており、`spring.session.data.redis.*` が正しい |
| **`repository-type: indexed` の明示** | 既定の `RedisSessionRepository` は `FindByIndexNameSessionRepository` を実装していないので、明示しないと**起動に失敗する** |
| **キースペース通知の有効化** | TTL はキー単位で Set の要素には張れないため、principal 索引の掃除に通知が要る。Redis 側の設定なので `notify-keyspace-events` が必要 |
| **`maxmemory-policy` の明示** | ElastiCache の既定 `volatile-lru` は TTL 付きキーを捨てる。セッションが黙って消えるので `noeviction` にする |
| **マイグレーション用タスクにも接続情報** | `indexed` は起動時に通知を購読しに行くため、Web でないタスクも Redis を要求する |
| **テスト環境の分離** | `@SpringBootTest` がログインを通すので、開発用 Redis にテストのセッションが混ざる |

**「③④の差は性能とインフラ費用だけ」は、できること(共有・失効・検索)については正しかった。**
増えたのは**設定の勘所**のほうで、予告の粒度では見えていなかった。

3 節に書いた「③のデメリット」のうち、実際に解消されたのは 2 番(負荷点が分かれた)と
3 番(掃除が `DELETE` から TTL に変わった)である。ただし 3 番は**完全には消えていない** —
Spring Session は索引の掃除のために**毎分 `EXISTS` を打つタスク**を回している。
形が変わっただけで、仕事そのものは残った。

詳細 → [docs/notes/redis/session-management.md](../../redis/session-management.md) /
[docs/notes/redis/elasticache.md](../../redis/elasticache.md)
...
このプロジェクトで言うと、**ログインの手続きは `SecurityConfig` の `formLogin()`(認証機能層)、セッションの置き場所は Spring Session(保管層)**と、担当が完全に分かれている。**実際にフェーズ18 で `spring-session-jdbc` を `spring-session-data-redis` に差し替えたが、ログイン処理のコードは 1 行も変わらなかった。**
