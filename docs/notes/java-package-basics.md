...
`package com.example.app;` という 1 行に詰まっている 2 つの疑問 — 「なぜ `com` から始まる?」「なぜ `backend/src/main/java` はパッケージ名に含まれない?」 — の答えをまとめた学習メモ。前半が**命名**(名前をどう付けるか)、後半が**配置**(名前とファイルの場所がどう結びつくか)、終盤が **main と test の関係**(なぜテストも同じパッケージ名なのか)の話。最後に、複数のクラスが**同じ / 別パッケージ**にいるときの **`import` の正体**と、「**なぜ機能ごとにパッケージを分けるのか(全部を同じ 1 つにしない理由)**」も扱う。
...
## 同じパッケージと別パッケージ — `import` の正体と「なぜ機能ごとに分けるのか」

ここまでは「1 つのクラスの住所」の話だった。実際のアプリでは `post` / `user` / `category` のように**複数のクラスが複数のパッケージに散らばって**互いを呼び合う。そのとき湧く 2 つの疑問 — 「他パッケージを呼ぶには `import` が要るのはなぜ?」「じゃあ全部同じパッケージにすれば `import` 不要で楽では?」 — に答える。

### パッケージ = フォルダと 1 対 1(`dto/` サブフォルダは別パッケージ)

ルールは 1 つだけ: **フォルダが違えば必ずパッケージも違う。** このプロジェクトの `post` まわりを見ると、同じ `post/` 直下は全部同じパッケージだが、一段掘った `post/dto/` は**別のパッケージ**になっている:

```
com/example/app/post/       → package com.example.app.post;      (Post, PostService, PostRepository, PostController)
com/example/app/post/dto/   → package com.example.app.post.dto;  (CreatePostRequest, PostResponse, TimelineResponse)
```

「`post` 配下は全部同じパッケージ」と思い込むと `dto/` で裏切られる。**フォルダを 1 つ掘ったら別パッケージ**、と覚えておく(この 1 対 1 対応の理由は前述「配置」の節)。

### `import` は「呼べるようにする」ためではない — 完全修飾名の略記

いちばん誤解しやすい点: **`import` が無くても他パッケージのクラスは呼べる。** 完全修飾名(パッケージ込みのフルネーム)で書けばいいだけ:

```java
// import 無しでも、これで普通に動く
com.example.app.category.Category c = new com.example.app.category.Category();
```

でも毎回この長い住所を書くのは大変なので、

```java
import com.example.app.category.Category;  // 「以後 Category と略記してよい」という宣言
...
Category c = new Category();                // 短く書ける
```

つまり **`import` は「呼べるようにする」ものではなく、完全修飾名を短い名前で書くための省略宣言**。呼べる / 呼べないを決めているわけではない。同じパッケージ内のクラス同士は住所が同じで略記の必要すらないから `import` が要らない、というだけ(前述の `ApplicationTests` が `Application` を `import` していないのもこの理由)。Laravel の `use App\Models\User;` も、完全修飾名 `\App\Models\User` を `User` と略記するための同じ役割の宣言。

### なぜ全部を 1 つのパッケージにまとめないのか

「同じパッケージなら `import` 不要で楽」は正しい直感。でもその楽さには 3 つの代償がある:

1. **名前が衝突する。** 同じパッケージの中には**同名のクラスを 2 つ置けない**。アプリが育つと「認証用の `User`」「管理画面用の `User`」のように同名クラスが欲しくなることがあり、パッケージを分ければ `auth.User` と `admin.User` として共存できるが、全部 1 つだと即アウト。
2. **「隠す」境界が消える(これが本質)。** package-private(前述「main と test」のアクセス修飾子表の第 3 段 = 修飾子を書かない状態)は「**同じパッケージからだけ見える**」という意味。全部を 1 つのパッケージにまとめると、この「同じパッケージだけ」という壁が消え、**隠したい内部処理をどこからでも触れてしまう**。パッケージを分けることは「触ってよい境界線を引く」ことでもあり、カプセル化(内部を隠して外には必要な分だけ見せる設計)の土台になっている。
3. **見通しが悪くなる。** `post` / `user` / `category` / `common` と機能単位で分かれているから「投稿まわりを直したい → `post` を見ればよい」と一目で分かる。全ファイルが 1 フォルダに並ぶ状態を想像すると、不便さがわかる。

まとめると: **`import` の手間(しかも IDE が自動補完するので実質ゼロ)を払って、名前空間の整理とアクセス制御の境界線を買っている。** 分けることには、楽さと引き換えにするだけの見返りがある。
...
- **import** — 完全修飾名を短い名前で略記するための宣言。クラスを「呼べるようにする」ものではない(import 無しでもフルネームで呼べる)。Laravel の `use` に相当
