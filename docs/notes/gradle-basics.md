...
### 「インストール」の正体 — gradle コマンド無しで用意でき、用意後も gradle コマンドが生えない理由

「gradle コマンドが使えないのに、どうやって Gradle をインストールできるの?」という疑問は、**「インストール」という言葉が実は 2 つの工程の合成**だと分解すると解ける:

1. **ファイルを置く** — zip をダウンロードして展開する
2. **名前で呼べるようにする** — PATH に登録して `gradle` と打てば起動する状態にする

Wrapper がやるのは **1 だけ**で、そこに gradle コマンドは要らない。ダウンロードと展開に必要なのは「HTTP でファイルを取れて zip を解凍できるプログラム」であり、その係が gradle-wrapper.jar という小さな Java プログラム。動かすのに必要なのは JDK の `java` だけで、gradlew スクリプトの実体も `java -jar gradle/wrapper/gradle-wrapper.jar` の 1 行(backend/gradlew:213 で実測)。

鶏と卵に見えるが、そもそも**「X をインストールするのに X が要る」ことはない**。npm を入れるとき npm は使わない(ブラウザで Node のインストーラをダウンロードする)。要るのは「ダウンロードして展開できる何か」で、ここではブラウザや curl の代役を wrapper jar が務めている。そして Gradle 本体も**ただの Java プログラム(jar の集まり)**なので、zip を展開した時点でもう完成品——コンパイルも登録処理も要らない。

#### 「使える状態にしない」のではなく「名前で呼べないだけ」(実測)

展開された Gradle には `bin/gradle` という本物の起動コマンドが**同梱されている**。フルパスで直接叩くと:

```
$ ~/.gradle/wrapper/dists/gradle-9.5.1-bin/<ハッシュ>/gradle-9.5.1/bin/gradle --version
Gradle 9.5.1          ← 普通に動く

$ which gradle
(見つからない)         ← PATH に登録されていないだけ
```

つまり「動かない状態で保管されている」のではなく、**完動品が PATH の外に置かれている**。なお Wrapper 自身はこの bin/gradle スクリプトすら経由せず、展開済みの jar を java に直接読み込ませて Gradle を起動する。

#### なぜあえて PATH に登録しないのか

1. **PATH の椅子は 1 つしかない。** 登録した瞬間「このマシンの gradle はこの版」という全プロジェクト共通の決め事が生まれ、Wrapper が倒したはずの「プロジェクトごとに要る版が違う」問題が復活する。倉庫 A には 9.5.1 と 10.0 が**並存**でき、どれを使うかは各プロジェクトの properties が毎回決める——という設計と PATH 登録は根本的に相性が悪い
2. **迂回路を作らない。** `gradle` と直接打てると、properties の版チェックを通らない抜け道ができる。「全員が必ず受付(gradlew)を通る」からこそ版の保証が成立する
3. **環境を汚さない。** PATH 登録はユーザーのシェル設定への書き込み。Wrapper は `~/.gradle` の中にファイルを置く以外に環境へ一切手を触れないので、痕跡を消すのも `rm -rf ~/.gradle`(このリポジトリなら `down -v`)だけで済む

まとめ: **OS へのインストール(工程 2)は最後まで起きない**。`~/.gradle/wrapper/dists/` へのダウンロード & キャッシュ(工程 1)だけであり、`gradle` コマンドは永遠に打てるようにならない——**常に `./gradlew` という受付係を経由**する。
...
- **インストールの 2 工程** — 「ファイルを置く(ダウンロード + 展開)」と「名前で呼べるようにする(PATH 登録)」。Wrapper は前者だけを行う
- **PATH 登録** — コマンド名だけでプログラムを起動できるようにする登録。椅子は実質 1 つなので「複数版の並存 + プロジェクトごとの選択」と相性が悪い
