...
- **工程の数がそもそも違う。** Java と PHP には⑥バンドルと⑦minify が**存在しない**。フロントエンドだけ 9 工程フルで走る。
...
**①④⑤⑧⑨は全言語共通で、③⑥⑦が言語によって出たり消えたりする。** これは偶然ではなく、③⑥⑦がすべて**「実行系に届けるための変換」**だから。実行系が何を受け付けるかで必要性が決まる。
...
**⑥⑦の位置が完全に空いている**のが特徴。SpotBugs だけ①パースのバーが無いのは、**ソースではなくコンパイル後のバイトコードを読む**ため。他のツールと入口が違う。
...
**フロントエンドとほぼ同じ形**になる。サーバーサイドなのに⑥⑦が必要なのが特徴。
...
**5. ④⑤の選択肢が最も多い。** ESLint + Prettier / Biome / oxlint + oxfmt / Deno 内蔵 / Bun 内蔵。**唯一「選ばなければならない」言語**。
...
- **wrangler** — Cloudflare Workers の CLI。内部で esbuild を使い③⑥⑦を担うが、**②型検査はしない**
