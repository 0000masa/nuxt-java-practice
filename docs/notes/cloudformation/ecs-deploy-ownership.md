...
`app.yml:1199` 付近(`AppTaskDefinition`)と `:1406` 付近(`Service`)。
...
`app.yml:1412` は `DesiredCount: !Ref WebDesiredCount` と書いており、ワークフローは毎回この値を渡す。**CloudFormation が ECS の `UpdateService` に `desiredCount` を常に含めるのか、変更のないプロパティは送らないのかが公式ドキュメントに明記されていない。**
