...
役割分担が重要です。**スキーマを作る・変えるのは Flyway だけ**。Hibernate(JPA)には `ddl-auto: update` のようにエンティティクラスから自動でテーブルを作る機能もありますが、このリポジトリでは `validate` にして「起動時にエンティティと実際のテーブルが一致しているか検証するだけ」に留めています。マイグレーションを書き忘れてエンティティだけ変えると起動時エラーで検出される、という二段構えです。
