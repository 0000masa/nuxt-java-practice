...
- **`LoggingLevel: ERROR` で `/aws/chatbot/...` にどれだけログが出るか。** CloudWatch Logs は課金対象なので、無ければ `NONE` に落とす
  - **追記(2026-09-06):量は未計測のままだが、前提が 2 つ変わった。** ロググループ名は `/aws/chatbot/<ConfigurationName>` で、**作られるのは us-east-1**(スタックのリージョンではない)。そして**コマンド実行の監査ログは常時有効で無効化できない**ので、`NONE` に落としてもロググループ自体は残る。フェーズ16 で承認を Slack から行うようになり、承認 = コマンド実行なので監査イベントは必ず出る。→ [Slack 手順書 §8-2](../../slack/README.md)
