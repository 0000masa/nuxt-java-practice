...
| 3 | **CSRF は有効。`XSRF-TOKEN` Cookie → `X-XSRF-TOKEN` ヘッダ** | Cookie セッション方式に必ず付いてくる対策。`SameSite` だけに頼らない |
...
| 7 | **メール送信は登録のコミット後**(`@TransactionalEventListener(AFTER_COMMIT)`) | DB トランザクションを SMTP の往復時間だけ開けたままにしない。メールサーバー障害で登録自体が失敗しない。送信失敗は再送で救う |
...
エンドポイントごとの詳細は実装時に `docs/api/` に 1 ファイルずつ追加する。
...
│   ├── AppUserDetails.java            … UserDetails 実装(users.id を保持)
│   ├── AppUserDetailsService.java     … メールアドレスで users を引く
│   ├── SignupCompletedEvent.java      … AFTER_COMMIT でメールを送るためのイベント
...
│   └── MailConfig.java                … 必要なら(application.yml で足りるなら作らない)
└── common/exception/
    └── GlobalExceptionHandler.java    … 認証系の例外 → 401 の変換を追加
```
...
| `post/PostService.java` | `create(request, userId)` / `delete(id, userId)` にシグネチャ変更。`userId` から `User` を引くのは `getReferenceById()` を使い、投稿作成時の無駄な SELECT を 1 回省く |
...
| `build.gradle` | security / session-jdbc / mail の依存を追加。**正確な artifact 名は Boot 4.1 で解決を確認する**(Boot 4 は自動設定がモジュール分割されており、フェーズ1 で `flyway-core` 単体では Flyway が有効にならなかった前例がある) |
...
| テスト | スライス | 本数 | 内容 |
|---|---|---|---|
| `AuthTokenServiceTest` | 実 MySQL(`app_test`) | 4 | 期限切れ / 使用済み / 存在しない値 / 別 purpose の流用が弾かれること |
| `AuthControllerTest` | `@WebMvcTest` | 数本 | メール形式・パスワード長・username 形式のバリデーションと `fieldErrors` |
| `AuthFlowTest` | `@SpringBootTest` + MockMvc | 1 | 登録 → 未確認ではログイン不可 → 確認 → ログイン成功。`JavaMailSender` は `@MockitoBean` で差し替え、送られた本文からトークンを取る |
| `PostControllerTest` | `@WebMvcTest` | 既存 + 1 | principal 対応 + 未ログイン 401 |
