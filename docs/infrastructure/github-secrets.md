...
**登録するのは 5 つ。すべて Environment `stg` の secret。** 置き場が分かれる理由は §1 に残してある(間違えると片方のワークフローだけが動かない)。

> **フェーズ16 で 1 つ減って 1 つ増えた。** 減ったのは `AWS_ECR_PUSH_ROLE_ARN`。`ecr-push.yml` 専用の **Repository** secret だったが、`main` ではイメージを作るのが CodeBuild になったので使わなくなった(→ [ADR-0013](../adr/0013-app-deploy-with-code-services.md))。
>
> 増えたのは `AWS_CODESTAR_CONNECTION_ARN`(→ §2-3)。**当初は `params/pipeline-stg.json` に平文で置く設計だったが、ARN にアカウント ID が入るのでやめた。**

**この 5 つは `main` の話。** [`github-actions-deploy` ブランチ](../adr/0012-deploy-method-per-branch.md)も動かせる状態にしてあるので、**Repository secret `AWS_ECR_PUSH_ROLE_ARN`(`nuxt-java-practice-gha-ecr-push` の ARN)と、その IAM ロールは消さずに残す。** 凍結ブランチの `ecr-push.yml` が使う唯一の Repository secret で、`main` のワークフローからは参照されない。**消すと凍結ブランチが `Credentials could not be loaded` で落ち、「動く記録」でなくなる。**

**AWS の長期クレデンシャル(アクセスキー)は 1 つも登録しない。** 4 つは ARN(IAM ロール 3 本 + CodeStar Connections 1 本)で、それ自体は資格情報ではない(→ [github-actions-oidc.md](./github-actions-oidc.md) §1)。本物の資格情報は `BASIC_AUTH_CREDENTIAL` だけ。**資格情報でないものを Secret に置いているのは、いずれも AWS アカウント ID を含むから**(→ §2-1・§2-3)。
...
|---|---|---|---|---|
| `AWS_CFN_DEPLOY_ROLE_ARN` | Environment `stg` | `nuxt-java-practice-gha-cfn-stg` の ARN | `cfn-apply.yml` / `cfn-destroy.yml` / `pipeline-apply.yml` / `pipeline-destroy.yml` | [cloudformation-operations.md](./cloudformation-operations.md) §2-2 |
...
| `AWS_CODESTAR_CONNECTION_ARN` | Environment `stg` | 握手済みの CodeStar Connections の ARN | `pipeline-apply.yml` | §2-3 |
...
**デプロイパイプラインの設定は、1 つだけここにある。** `params/pipeline-<env>.json` に置いてあるのはリポジトリ名・ブランチ名・Slack のチャンネル ID までで、**CodeStar Connections の ARN だけは Secret**(→ §2-3)。
...
| `cfn-apply.yml` / `db-task.yml` / `pipeline-apply.yml` | `environment: ${{ inputs.env }}` | Repository + Environment `stg` |
| `cfn-destroy.yml` | `environment: stg` | 同上 |
| `pipeline-destroy.yml` | `environment: ${{ inputs.env }}` | 同上 |
...
| 5 つを Repository secret に置いた | **動いてしまう。** Repository secret は Environment を宣言したジョブからも読めるため。ただし prod を足した瞬間に stg / prod で値を分けられなくなるので、そのときに全部やり直しになる |
...
### 2-3. CodeStar Connections の ARN

パイプラインが GitHub からソースを取るための接続(→ [cloudformation-operations.md](./cloudformation-operations.md) §12-1)。**コンソールで作って GitHub と握手を済ませてから**控える。
...
`arn:aws:codeconnections:ap-northeast-1:<アカウントID>:connection/<uuid>` の形。**作成時期によって `codeconnections` と `codestar-connections` のどちらかになる**(AWS のサービス改名の名残)。出力をそのまま入れればよく、綴りを直す必要はない。

**この値を `params/pipeline-<env>.json` に書かないこと。** アカウント ID を含むうえ、`pipeline-apply.yml` は params を `--parameter-overrides` に展開するので **public リポジトリの Actions ログにも出てしまう**。Secret 経由なら両方とも `***` にマスクされる。ロールの ARN を Secret に置いているのとまったく同じ理由(→ §2-1)。

**接続は環境をまたいで共通でよい。** リポジトリ単位の握手なので、prod を足しても同じ ARN を Environment `prod` に入れ直すだけ。
...
**protection rules は設定しない**(というより設定できない)。GitHub Free のプライベートリポジトリでは required reviewers もブランチ制限も使えないので、**ブランチ制限は IAM の信頼ポリシー側**(`token.actions.githubusercontent.com:ref` の条件)で掛けている。許しているのは `main` と `github-actions-deploy` の 2 本(→ [cloudformation-operations.md](./cloudformation-operations.md) §2-2)。
...
### 3-3. Environment secrets(5 つ)

作った `stg` の画面の **Environment secrets → Add environment secret** で 5 つ登録する。
...
| `AWS_CODESTAR_CONNECTION_ARN` | 握手済みの接続の ARN(§2-3) |
...
**ワークフローのコードは 1 行も変えない。** `cfn-apply.yml` と `db-task.yml` は `environment: ${{ inputs.env }}` なので、Environment `prod` を作って**同じ 5 つの名前**で `-prod` のロールの ARN を入れれば切り替わる。これが Environment secrets にしている理由。

必要になるもの:

- IAM ロール 3 本(`...-gha-cfn-prod` / `...-cfn-service-prod` / `...-gha-dbtask-prod`)。**信頼ポリシーの `sub` は `environment:prod` に変える**
- Environment `prod` と、その Environment secrets 5 つ(`AWS_CODESTAR_CONNECTION_ARN` は **stg と同じ値でよい** → §2-3)
...
| `pipeline-apply` が「secret AWS_CODESTAR_CONNECTION_ARN が空です」で落ちる | 登録漏れか名前違い。**Repository secret に入れていないか**も確認する(`pipeline-apply.yml` は `environment:` を宣言しているので読めはするが、prod で値を分けられなくなる → §1) |
